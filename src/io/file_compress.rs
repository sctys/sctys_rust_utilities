...
impl<'a> FileCompress<'a> {
    ...
    pub fn tar_additional_file<W: Write>(
        ...
        match File::open(&full_path) {
            Ok(mut file) => match builder.append_file(&full_archive_path, &mut file) {
                Ok(_) => {
                    let debug_str =
    ...
    pub fn tar_additional_folder<W: Write>(
        ...
        match builder.append_dir_all(archive_path, folder_path) {
            Ok(_) => {
    ...
    pub fn run_compression<W: Write>(&self, builder: &mut Builder<W>) {
...
mod tests {
    ...
    fn test_tar_files() {
        ...
        for file in file_list {
            file_compress.tar_additional_file(&folder_path, archive_path, &file, &mut compressor);
        }
        file_compress.run_compression(&mut compressor);
    }
    ...
    fn test_tar_folder() {
        ...
        let mut compressor = file_compress.get_bz2_compressor(&folder_path, &compressed_file_name);
        file_compress.tar_additional_folder(
