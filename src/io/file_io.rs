...
impl<'a> FileIO<'a> {
    ...
    pub fn create_directory_if_not_exists(&self, folder_path: &PathBuf) {
        if !folder_path.is_dir() {
            match fs::create_dir_all(folder_path) {
                ...
                Err(e) => {
                    let error_str = format!("Unable to create folder {}. {e}", folder_path.display());
    ...
    fn get_last_modification_time(&self, full_path: &PathBuf) -> SystemTime {
        match fs::metadata(full_path) {
            Ok(m_d) => match m_d.modified() {
                Ok(m_t) => m_t,
                Err(e) => {
                    let error_str = format!("Unable to get the last modification time for {}, {e}", full_path.display());
                    self.project_logger.log_error(&error_str);
                    panic!("{}", &error_str);
                }
            },
    ...
    pub fn get_elements_in_folder(&self, folder_path: &PathBuf) -> ReadDir {
        let elements = match fs::read_dir(folder_path) {
            Ok(r_d) => r_d,
            Err(e) => {
                let error_str = format!("Unable to get the elements in folder {}, {e}", folder_path.display());
                self.project_logger.log_error(&error_str);
                panic!("{}", &error_str)
            }
        };
    ...
    pub fn filter_element_after<T: TimeZone>(&self, element: &Result<DirEntry>, cutoff_date_time: DateTime<T>) -> bool {
    ...
    pub fn filter_element_between<T: TimeZone>(&self, element: &Result<DirEntry>, cutoff_date_time_early: DateTime<T>, cutoff_date_time_late: DateTime<T>) -> bool {
        let dir_entry = match element {
            Ok(d_e) => d_e,
            Err(e) => panic!("Unable to identify the element. {e}")
        };
        let full_path = dir_entry.path();
        let modified_time = self.get_last_modification_time(&full_path);
        (time_operation::diff_system_time_date_time_sec(modified_time, cutoff_date_time_early) >= 0) && (time_operation::diff_system_time_date_time_sec(modified_time, cutoff_date_time_late) < 0)
    }
    ...
    pub fn load_file_as_string(&self, folder_path: &PathBuf, file: &String) -> String {
        ...
        match fs::read_to_string(&full_path) {
            ...
            Err(e) => {
                let error_str = format!("Unable to load file {} as string. {e}", &full_path.display());
                self.project_logger.log_error(&error_str);
                panic!("{}", &error_str)
            }
        }
    ...
    pub fn write_string_to_file(&self, folder_path: &PathBuf, file: &String, content: &String) {
        ...
        match fs::write(&full_path, content) {
            ...
            Err(e) => {
                let error_str = format!("Unable to save string to file {}. {e}", &full_path.display());
                self.project_logger.log_error(&error_str);
                panic!("{}", &error_str);
            }
        };
    ...
    pub fn load_csv_file(&self, folder_path: &PathBuf, file: &String) -> DataFrame {
        ...
        match csv_reader.has_header(true).finish() {
            Ok(df) => df,
            Err(e) => panic!("Unable to convert csv file {}/{file} into data frame. {e}", folder_path.display())
        }
    }
    ...
    pub fn write_csv_file(&self, folder_path: &PathBuf, file: &String, data: &mut DataFrame) {
        ...
        if let Err(e) = csv_writer.has_header(true).with_delimiter(b',').finish(data) {
            let error_str = format!("Unable to write csv file {}/{file}. {e}", folder_path.display());
            self.project_logger.log_error(&error_str);
            panic!("{}", &error_str)
        }
    }
    ...
    pub fn load_parquet_file(&self, folder_path: &PathBuf, file: &String) -> DataFrame {
        ...
        match parquet_reader.finish() {
            Ok(df) => df,
            Err(e) => panic!("Unable to convert parquet file {}/{file} into data frame. {e}", folder_path.display())
        }
    }
    ...
    pub fn write_parquet_file(&self, folder_path: &PathBuf, file: &String, data: &mut DataFrame) {
        ...
        if let Err(e) = parquet_writer.finish(data) {
            let error_str = format!("Unable to write parquet file {}/{file}. {e}", folder_path.display());
            self.project_logger.log_error(&error_str);
            panic!("{}", &error_str)
        }
    }
...
mod tests{
    ...
    fn test_folder_exist() {
        let folder_path = Path::new(&env::var("SCTYS_PROJECT").unwrap()).join("sctys_rust_utilities");
    ...
    fn test_create_directory_if_not_exist() {
        let folder_path = Path::new(&env::var("SCTYS_PROJECT").unwrap()).join("sctys_rust_utilities").join("abc");
        assert!(!FileIO::check_folder_exist(&folder_path));
        let logger_name = "test_file_io";
        let logger_path = Path::new(&env::var("SCTYS_PROJECT").unwrap()).join("Log").join("log_sctys_io");
    ...
    fn test_filter_file_modified_after() {
        let folder_path = Path::new(&env::var("SCTYS_PROJECT").unwrap()).join("Log").join("log_sctys_notify");
        let logger_name = "test_file_io";
        let logger_path = Path::new(&env::var("SCTYS_PROJECT").unwrap()).join("Log").join("log_sctys_io");
        let project_logger = ProjectLogger::new_logger(&logger_path, logger_name);
        let _handle = project_logger.set_logger();
    ...
    fn test_filter_file_modified_between() {
        ...
        let file_list = elements.filter(|x| file_io.filter_element_between(x, cutoff_date_time_early, cutoff_date_time_late));
